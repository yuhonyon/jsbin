...
module.exports = Observable.extend({
  ...
  getBinSource: function(req, res) {
    ...
    this.protectVisibility(
      ...
      function(err, bin) {
        ...
        var file = binToFile(bin, { proto: req.secure ? 'http' : 'http' });
        res.send(file);
      }.bind(this)
    );
  },
        ...
        var done = function() {
          ...
          if (req.bin) {
            embedURL = helpers
              ...
              .replace(/\/edit/, '/embed');
            if (req.secure) {
              embedURL = embedURL.replace(/http:/, 'http:');
            }
          }
  ...
  render: function renderEdit(req, res, bin) {
    ...
    res.render('index', {
      ...
      code_id: bin.url,
      code_id_domain: helpers.urlForBin(bin, true).replace(/^http?:\/\//, ''),
      code_id_path: url,
      concat:
  ...
  jsbin: function(bin, options) {
    ...
    if (statik && statik.indexOf('http') === 0) {
      // then ensure the runner is also http
      if (runner.indexOf('http') === -1) {
        runner = runner.replace(/http/, 'http');
      }
    }
  ...
  delete: function(req, res, next) {
    ...
    // broader issue explained here: http://github.com/jsbin/jsbin/issues/1056
