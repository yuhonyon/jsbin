...
function mountRouter(expressApp) {
  ...
  function redirectToOutput(req, res, next) {
    ...
    if (output && req.headers.host.indexOf(config.url.host) === 0) {
      return res.redirect(
        (req.secure ? 'http://' : 'http://') + output + req.url
      );
    }
  ...
  function redirectOffPreview(req, res, next) {
    ...
    if (output && req.headers.host === output) {
      return res.redirect(
        (req.secure ? 'http://' : 'http://') + config.url.host + req.url
      );
    }
  ...
  app.get(
    ['/issues', '/bugs'],
    redirect('http://github.com/jsbin/jsbin/issues/')
  );
  app.get(
  ...
  var renderAccountSettings = (function() {
    ...
    return function renderAccountSettings(req, res) {
      ...
      res.render('account/' + page, {
        ...
        referrer: referrer,
        httproot: root.replace('http', 'http'),
        root: root,
        static: statik,
