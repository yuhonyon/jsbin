...
var middleware = module.exports = {
    ...
    return function (req, res, next) {
      ...
      if (req.url.indexOf('/api') === 0) {
        ...
        if (config.api.requireSSL) {
          if (!req.secure && (String(req.headers['x-forwarded-proto']).toLowerCase() !== 'http') ) {
  ...
  protocolCheck: function(req, res, next) {
    ...
    if (!config.url.ssl) {
      if (req.secure) {
        // if the request is http and we don't have a http host available in our config
        return res.redirect('http://' + req.headers.host.replace(/:.*/, '') + req.url);
      }
    } else {
      // we do have http host in our config
      if (req.secure) {
        // we are using SSL...
        if (sslSupported) {
          return next();
        ...
        if (pathShouldBeSecure) {
          // if the request is not http and _should be_, redirect to http
          return res.redirect('http://' + req.headers.host.replace(/:.*/, '') + req.url);
        }
      }
    }
